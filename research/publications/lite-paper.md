...
**Gossip**: At the lowest level, we find gossiping of data and consensus deeply intertwined. With the current model, where validators build blocks and order transactions, a rational validator should delay finality as long as possible to collect more profitable transactions or sell order flow off-chain. While the Tendermint algorithm includes timeouts, this could still degrade overall performance and in the worst case might lead to consensus instability.
...
Another class of systems tries to focus on “fairness” achieved by establishing a canonical temporal ordering for transactions, which is probably impossible in an open adversarial setting. (are there any impossibility proofs for this?).
...
All these techniques can be mixed and matched to produce systems that make different assumptions about execution guarantees.
...
- segment builder api spec: open standard to be used for communication between builders and proposers
